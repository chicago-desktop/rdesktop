...
local function define_tests()
    test.describe("the loopback transport", function()
        ...
        test.it("tells a producer in a viewport the viewer's terminal, not the process's", function()
            -- The contract the picture path stands on: view:terminal before
            -- the producer starts is what the producer's gfx hears (runtime
            -- 91dcbee8). Sixel, which no environment guesses here.
            local view: any = assert(tty.viewport({width = 20, height = 5}))
            test.is_true(view:terminal("sixel", 8, 20), "integer sizes are accepted")
            local replies = process.listen("cellprobe")
            assert(process.with_options({terminal = assert(view:grant())})
                :spawn("app:cellprobe", "app:processes", tostring(process.pid())))
            local picked = channel.select({replies:case_receive(), time.after("5s"):case_receive()})
            process.unlisten(replies)
            test.eq(tostring(picked.value), "before=sixel/nil 8/20 | after=sixel/nil 8/20")
            view:close()
        end)
